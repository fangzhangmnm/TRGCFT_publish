...
from LTRG import mysvd, myeigh, myeig_old
from LTRG import get_linearized_HOTRG_autodiff, get_linearized_HOTRG_full_autodiff
from LTRG import get_linearized_cylinder, verify_linear_operator, check_hermicity
from LTRG import get_central_charge_from_spectrum,get_scaling_dimensions_from_spectrum
from HOTRG import HOTRG_layers



print('loading tensors...')
tensor_data=torch.load(options['tensor_path'],map_location=device)
options1,params,layers,Ts,logTotals=tensor_data['options'],tensor_data['params'],tensor_data['layers'],tensor_data['Ts'],tensor_data['logTotals']
...
if options['linearized_full']:
    ...
    layers_sel=HOTRG_layers(T,max_dim=T.shape[0],nLayers=2,options=options)[0]
...
print(get_scaling_dimensions_from_spectrum(torch.as_tensor(sr).abs(),scaling=2))
