...
for MAX_DIM in "${CHIS[@]}"
do
    python run_find_critical_temp.py --filename data/hotrg_gilt_chis/X${MAX_DIM}/Tc.pth --log_filename data/hotrg_gilt_chis/X${MAX_DIM}/Tc.txt --nLayers ${NLAYERS} --max_dim ${MAX_DIM} --model Ising2D --param_name beta --param_min 0.440683 --param_max 0.440685 --observable_name magnetization --gilt_enabled --gilt_eps ${GILT_EPS} --mcf_enabled --tol 1e-8 --method obs --device ${DEVICE}
